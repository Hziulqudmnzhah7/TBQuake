...
//		G_Printf("serverTime >>>>>\n" );
	...
	/*TBQoct2*planetGrav*/
	VectorSubtract(client->ps.origin, plans[0], tgrav);
	if (VectorLength(tgrav) > dist)
	{
		dist = VectorLength(tgrav);
	}
	VectorScale(tgrav, g * (masses[0] / (dist*dist*dist)) * (level.time - t) * (level.time - t), tgrav);
	VectorAdd(grav, tgrav, grav);
	//if (
	VectorAdd(client->ps.velocity, grav, client->ps.velocity);
	t = level.time;
	//*LineDraw*****/
	//tent = G_TempEntity( plans[0], EV_RAILTRAIL );

	// set player number for custom colors on the railtrail
	//tent->s.clientNum = ent->s.clientNum;

	//VectorCopy(client->ps.origin, tent->s.origin2 );
	/////////////////////////////////////////////////////////////////
