...
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	bolt->r.ownerNum = self->s.number;
	bolt->parent = self;
	/*TBQsept27*rocketdmg 1*disabled*/
	bolt->damage = 100;
	bolt->splashDamage = 100;
...
//************************TBQsept27*fast rocket***ammoTied**************************************
	//*
	if (self->client)
	{
		VectorScale(dir, -10, bolt->s.pos.trDelta );
		//****************TBQsept28*knockbackfire*******/
		 		//VectorScale(dir, -1, dir);
		VectorScale (dir, 6001 / 200, dir);
		VectorScale (dir, 6001 / 200, dir);
		VectorAdd (self->client->ps.velocity, dir, self->client->ps.velocity);
	}
